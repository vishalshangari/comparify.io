...
router.get("/user-info/:type", async (req, res) => {
  const { type } = req.params;
});

router.get("/saved-data/:id", async (req, res, next) => {
  const requestedUserID = req.params.id;
  console.log(JSON.stringify(req.cookies));
  try {
    const { _id: activeUserID } = jwt.verify(
      req.cookies["comparifyToken"],
      process.env.JWT_SECRET
    );
    // res.cookie(SERVER_PRIVATE_KEY, process.env.SERVER_SECRET);
    const reqUserID = requestedUserID || activeUserID;
    const response = await axios.get(`/private/saved-data/${reqUserID}`, {
      headers: {
        Cookie: `${SERVER_PRIVATE_KEY}=${process.env.SERVER_SECRET};`,
      },
    });
  } catch (error) {
    console.error(error);
...
router.get("/private/saved-data/:id", async (req, res) => {
  if (req.cookies[SERVER_PRIVATE_KEY] !== SERVER_SECRET) {
...
router.post("/artist-info/multiple", async (req, res) => {
  ...
  try {
    ...
    console.log(error);
    res.send(`There was an error getting your top artists.`);
  }
});
